...
const NoteMenuSetup = ({changeHandler}) => {
  return (
    <div className='absolute rounded-sm justify-center mt-16 w-full flex'>
      ...
      <p className='absolute mb-44 ml-2 title text-5xl'>Enter Note <span onClick={changeHandler} className="material-symbols-outlined absolute ml-14 -top-2 text-4xl hover:cursor-pointer clicked">cancel</span></p>
        ...
        <button className='slightContrastBg clicked rounded-lg p-4 title text-4xl mt-4 items-center' onClick={sendForm} id='submitButtonHTMLElement'>Add</button>
      </div>
    </div>
  )
}
